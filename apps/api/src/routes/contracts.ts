...
router.post("/", requireAuth, (req, res) => {
  ...
  const currentUser = db.prepare("SELECT name FROM users WHERE id = ?").get(req.user!.userId) as { name: string }
  insertParty.run(req.user!.userId, contractId, currentUser.name, "promisor", now)

  for (const p of parties) {
    insertParty.run(p.id || uuid(), contractId, p.name, p.role, now)
  }
  ...
  db.prepare("UPDATE users SET total_contracts = total_contracts + 1 WHERE id = ?").run(req.user!.userId)
  for (const p of parties) {
    if (p.id && p.id.startsWith("u")) {
      db.prepare("UPDATE users SET total_contracts = total_contracts + 1 WHERE id = ?").run(p.id)
    }
  }
...
router.patch("/:id", requireAuth, (req, res) => {
  ...
  if (status === "completed" && existing.status !== "completed") {
    const parties = db.prepare("SELECT * FROM parties WHERE contract_id = ?").all(param(req.params.id)) as any[]
...
router.patch("/:id/clauses/:clauseId", requireAuth, (req, res) => {
  ...
  const clause = db.prepare("SELECT * FROM clauses WHERE id = ? AND contract_id = ?").get(param(req.params.clauseId), param(req.params.id)) as any
  if (!clause) { res.status(404).json({ error: "Clause not found" }); return }

  const { status } = req.body
  if (!status) { res.status(400).json({ error: "status is required" }); return }

  db.prepare("UPDATE clauses SET status = ? WHERE id = ?").run(status, param(req.params.clauseId))
  ...
  if (allDone) {
    db.prepare("UPDATE contracts SET status = 'completed', updated_at = datetime('now') WHERE id = ?").run(param(req.params.id))
    const parties = db.prepare("SELECT * FROM parties WHERE contract_id = ?").all(param(req.params.id)) as any[]
    const userIds = new Set(parties.filter((p: any) => p.id.startsWith("u")).map((p: any) => p.id))
    for (const uid of userIds) {
      db.prepare("UPDATE users SET fulfilled_contracts = fulfilled_contracts + 1, trust_score = MIN(100, trust_score + 10) WHERE id = ?").run(uid)
    ...
    db.prepare("UPDATE contracts SET status = 'breached', updated_at = datetime('now') WHERE id = ?").run(param(req.params.id))
    const parties = db.prepare("SELECT * FROM parties WHERE contract_id = ?").all(param(req.params.id)) as any[]
    const userIds = new Set(parties.filter((p: any) => p.id.startsWith("u")).map((p: any) => p.id))
    for (const uid of userIds) {
      db.prepare("UPDATE users SET breached_contracts = breached_contracts + 1, trust_score = MAX(0, trust_score - 15) WHERE id = ?").run(uid)
